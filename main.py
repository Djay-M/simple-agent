...
async def main():
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            print("Avaiable Tools = ", *[tool.name for tool in tools])
            print("*" * 60)

            while True:
                user_input = input("\n You: ")

                if user_input.lower() in ["bye", "quit", "goodbye"]:
                    print("Good Bye")
                    break

                messages.append({"role": "user", "content": user_input[:1000]})
                ...
                try:
                    ...
                    print("Error while calling the ai agent error:", error)
